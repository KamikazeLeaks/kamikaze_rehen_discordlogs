PerformHttpRequest(Config.WebHook, function(err, text, headers) end, 'POST', json.encode({username = Config.DiscordName, content = "Discord Webhook is **ONLINE**", Config.AvatarURL = Config.ImageURL}), { ['Content-Type'] = 'application/json' })


local hostageAllowedWeapons = {
	"WEAPON_PISTOL",
	"WEAPON_COMBATPISTOL",
	-- Poner las armas que deseas que se puedan usar
}
...
RegisterCommand("rehen",function() -- Comando
	takeHostage()
    PerformHttpRequest(Config.WebHook, function(err, text, headers) end, 'POST', json.encode({username = Config.DiscordName, content = "Una Persona ha utilizado el comando REHEN", Config.AvatarURL = Config.ImageURL}), { ['Content-Type'] = 'application/json' })
end)

RegisterCommand("rh",function() -- Sub Comando
	takeHostage()
end)
...
function takeHostage()
	...
	if not canTakeHostage then 
		drawNativeNotification("Necesitas una pistola con municion para poder hacerlo") -- Notificacion
	end

	if not holdingHostageInProgress and canTakeHostage then	 -- Animacion	
		...
		if closestPlayer ~= -1 and closestPlayer ~= nil then
			...
			drawNativeNotification("No hay nadie cerca para hacerlo") -- Notificacion al no poder hacerlo
		end 
	end
...
Citizen.CreateThread(function()
	while true do 
		if holdingHostage then
			...
			DrawText3D(playerCoords.x,playerCoords.y,playerCoords.z,"[G] para liberarlo, [H] para matarlo")
